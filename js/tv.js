...
const TV = {
  ...
  expand() {
    ...
    // Click anywhere outside TV bezel to close
    // Using onclick directly — replaced each time expand() is called
    overlay._closeHandler = (e) => {
      if (!e.target.closest('.bc-tv-big-wrap, .sro-expand-copy, .bc-tv-close')) {
        this.collapse();
      }
    };
  ...
  collapse() {
    ...
    // Remove click handler to prevent stacking
